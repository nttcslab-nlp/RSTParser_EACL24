...
class DataArguments:
    ...
    dataset_dir: str = field(
        default="preprocessed_data/rstdt",
        metadata={"help": "Path to the dataset directory."},
    )
    train_file: str = field(
        default="span_train.json", metadata={"help": "Name of the training file."}
    )
    valid_file: str = field(
        default="span_valid.json", metadata={"help": "Name of the validation file."}
    )
    test_file: str = field(
        default="span_test.json", metadata={"help": "Name of the test file."}
    )
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_dir, train_file, valid_file, test_file):
        if os.path.exists(dataset_dir):
            try:
                args.dataset_format = (
                    args.dataset_format if args.dataset_format else "input-output"
                )
                full_dataset = DatasetDict.from_json(
                    {
                        "train": os.path.join(dataset_dir, train_file),
                        "eval": os.path.join(dataset_dir, valid_file),
                        "test": os.path.join(dataset_dir, test_file),
                    }
                )
                return full_dataset
            except:
                raise ValueError(f"Error loading dataset from {dataset_dir}")
        else:
            raise NotImplementedError(f"Dataset {dataset_dir} not implemented yet.")

    def format_dataset(dataset, dataset_format):

        assert (
            dataset_format == "input-output"
        ), "Only input-output format is supported for now."
    ...
    # Load dataset.
    dataset = load_data(
        args.dataset_dir, args.train_file, args.valid_file, args.test_file
    )
